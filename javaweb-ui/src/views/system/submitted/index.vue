  ...
  <div class="ele-body">
    <el-card shadow="never">
      ...
      <div class="ele-table-tool ele-table-tool-default">
        <el-button @click="showEdit=true" type="primary" icon="el-icon-plus" class="ele-btn-icon" size="small" v-if="permission.includes('sys:submitted:add')">添加
        </el-button>
        <el-button @click="remove()" type="danger" icon="el-icon-delete" class="ele-btn-icon" size="small" v-if="permission.includes('sys:submitted:dall')">批量删除
        </el-button>
      </div>
      <!-- 数据表格 -->
      <ele-data-table ref="table" :config="table" :choose.sync="choose" height="calc(100vh - 315px)" highlight-current-row>
        <template slot-scope="{index}">
          ...
          <el-table-column label="操作" width="130px" align="center" :resizable="false"  fixed="right">
            <template slot-scope="{row}">
              <el-link @click="edit(row)" icon="el-icon-edit" type="primary" :underline="false" v-if="permission.includes('sys:submitted:edit')">修改</el-link>
              <el-popconfirm title="确定要删除此信息上报吗？" @confirm="remove(row)" class="ele-action">
                <el-link slot="reference" icon="el-icon-delete" type="danger" :underline="false" v-if="permission.includes('sys:submitted:delete')">删除</el-link>
              </el-popconfirm>
            </template>
          </el-table-column>
        </template>
    ...
    <!-- 编辑弹窗 -->
    <el-dialog :title="editForm.id?'修改信息上报':'修改信息上报'" :visible.sync="showEdit" width="400px"
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        table: {url: '/submitted/index', where: {}},  // 表格配置
        choose: [],  // 表格选中数据
    ...
    methods: {
      /* 显示编辑 */
      edit(row) {
        this.$http.get('/submitted/info/' + row.id).then(res => {
          if (res.data.code === 0) {
      ...
      save() {
        this.$refs['editForm'].validate((valid) => {
          if (valid) {
            ...
            this.$http[this.editForm.id ? 'put' : 'post'](this.editForm.id ? '/submitted/edit' : '/submitted/add', this.editForm).then(res => {
              loading.close();
              if (res.data.code === 0) {
                this.showEdit = false;
      ...
      remove(row) {
        if (!row) {  // 批量删除
          ...
          this.$confirm('确定要删除选中的信息上报吗?', '提示', {type: 'warning'}).then(() => {
            const loading = this.$loading({lock: true});
            this.$http.delete('/submitted/delete/' + ids).then(res => {
              loading.close();
              if (res.data.code === 0) {
                this.$message({type: 'success', message: res.data.msg});
                this.$refs.table.reload();
          ...
          const loading = this.$loading({lock: true});
          this.$http.delete('/submitted/delete/' + [row.id]).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.$message({type: 'success', message: res.data.msg});
              this.$refs.table.reload();
      ...
      setStatus(row) {
        ...
        this.$http.put('/submitted/setStatus', params).then(res => {
          loading.close();
          if (res.data.code === 0) {
            this.$message({type: 'success', message: res.data.msg});
          } else {
