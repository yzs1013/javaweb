    ...
    <el-card shadow="never">
      ...
      <ele-data-table ref="table" :config="table" :choose.sync="choose" height="calc(100vh - 315px)" highlight-current-row>
        <template slot-scope="{index}">
          ...
          <el-table-column prop="name" label="友链名称" sortable="custom" show-overflow-tooltip min-width="200"/>
          ...
          <el-table-column prop="url" label="友链URL" sortable="custom" show-overflow-tooltip min-width="200"/>
          <el-table-column label="友链图片" min-width="100" align="center">
            <template slot-scope="{row}">
              <el-avatar shape="square" :size="35" :src="row.image"/>
            </template>
          </el-table-column>
          <el-table-column prop="form" label="友链形式" sortable min-width="150">
            <template slot-scope="{row}">
              <ele-dot :type="['', 'success'][row.form-1]" :ripple="row.form===0"
                        :text="['文字链接','图片链接'][row.form-1]"/>
            </template>
          </el-table-column>
          <el-table-column prop="status" label="友链状态" sortable="custom" :resizable="false" min-width="120">
            <template slot-scope="{row}">
    ...
    save() {
      this.$refs['editForm'].validate((valid) => {
        if (valid) {
          ...
          this.$http[this.editForm.id ? 'put' : 'post'](this.editForm.id ? '/link/edit' : '/link/add', this.editForm).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.showEdit = false;
    ...
    remove(row) {
      if (!row) {  // 批量删除
        ...
        this.$confirm('确定要删除选中的友链吗?', '提示', {type: 'warning'}).then(() => {
          const loading = this.$loading({lock: true});
          this.$http.delete('/link/delete/' + ids).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.$message({type: 'success', message: res.data.msg});
              this.$refs.table.reload();
