...
<el-table-column prop="medtype" label="媒体类型" align="center" min-width="100">
          ...
          <el-table-column label="舆情类型" show-overflow-tooltip min-width="200" align="center">
            <template slot-scope="{row}">
              <el-tag v-for="item in row.yuqingtype" :key="item.id" type="primary" size="mini">{{
                  item.name
                }}
              </el-tag>
            </template>
...
<el-table-column prop="grade" label="等级舆情" align="center" min-width="100">
          ...
          <el-table-column label="报送时间" sortable="custom" show-overflow-tooltip min-width="160" align="center">
            <template slot-scope="{row}">{{ row.{colunm_name} | toDateString }}</template>
          </el-table-column>
          <el-table-column label="刊发时间" sortable="custom" show-overflow-tooltip min-width="160" align="center">
            <template slot-scope="{row}">{{ row.{colunm_name} | toDateString }}</template>
          </el-table-column>
          ...
          <el-table-column prop="special_id" label="专题选择" align="center" show-overflow-tooltip min-width="200"/>
...
<el-table-column prop="city" label="所属地域" align="center" min-width="100">
          ...
          <el-table-column prop="is_deleted" label="是否已删除" align="center" show-overflow-tooltip min-width="200"/>
<el-table-column prop="status" label="是否启用" align="center" min-width="100">
              <template slot-scope="{row}">
                <ele-dot :type="['success', 'danger', 'primary', 'warning', 'info'][row.status-1]" :ripple="row.status===0"
                         :text="['启用','停用',][row.status-1]"/>
              </template>
            </el-table-column>
          <el-table-column prop="create_user" label="添加人" align="center" show-overflow-tooltip min-width="200"/>

          <el-table-column prop="update_user" label="更新人" align="center" show-overflow-tooltip min-width="200"/>
...
this.getmedtypeList(); // 查询媒体类型列表
this.getyuqingtypeList(); // 查询舆情类型列表
this.getinfotypeList(); // 查询信息类型列表
this.getgradeList(); // 查询等级舆情列表
this.getsendtimeList(); // 查询报送时间列表
this.getfiletimeList(); // 查询刊发时间列表



this.getdomaintypeList(); // 查询领域类型列表


this.getsendtoaList(); // 查询发送对象列表




this.getcityList(); // 查询所属地域列表
...
this.getstatusList(); // 查询是否启用列表

this.getcreateTimeList(); // 查询添加时间列表

this.getupdateTimeList(); // 查询更新时间列表
...
,
     ...
     */
    getmedtypeList() {
      this.$http.get('/level/getmedtypeList').then(res => {
        if (res.data.code === 0) {
          this.medtypeList = res.data.data;
        } else {
          this.$message.error(res.data.msg);
        }
      }).catch(e => {
...
,
     ...
     */
    getyuqingtypeList() {
      this.$http.get('/level/getyuqingtypeList').then(res => {
        if (res.data.code === 0) {
          this.yuqingtypeList = res.data.data;
        } else {
          this.$message.error(res.data.msg);
        }
      }).catch(e => {
...
,
     ...
     */
    getinfotypeList() {
      this.$http.get('/level/getinfotypeList').then(res => {
        if (res.data.code === 0) {
...
,
     ...
     */
    getgradeList() {
      this.$http.get('/level/getgradeList').then(res => {
        if (res.data.code === 0) {
...
,
     ...
     */
    getsendtimeList() {
      this.$http.get('/level/getsendtimeList').then(res => {
        if (res.data.code === 0) {
...
,
     ...
     */
    getfiletimeList() {
      this.$http.get('/level/getfiletimeList').then(res => {
        if (res.data.code === 0) {
          this.filetimeList = res.data.data;
        } else {
          this.$message.error(res.data.msg);
        }
      }).catch(e => {
...
,
     ...
     */
    getdomaintypeList() {
      this.$http.get('/level/getdomaintypeList').then(res => {
        if (res.data.code === 0) {
          this.domaintypeList = res.data.data;
        } else {
          this.$message.error(res.data.msg);
        }
      }).catch(e => {
...
,
     ...
     */
    getsendtoaList() {
      this.$http.get('/level/getsendtoaList').then(res => {
        if (res.data.code === 0) {
          this.sendtoaList = res.data.data;
        } else {
          this.$message.error(res.data.msg);
        }
      }).catch(e => {
...
,
     ...
     */
    getcityList() {
      this.$http.get('/level/getcityList').then(res => {
        if (res.data.code === 0) {
...
,
     ...
     */
    getstatusList() {
      this.$http.get('/level/getstatusList').then(res => {
        if (res.data.code === 0) {
...
,
     ...
     */
    getcreateTimeList() {
      this.$http.get('/level/getcreateTimeList').then(res => {
        if (res.data.code === 0) {
...
,
     ...
     */
    getupdateTimeList() {
      this.$http.get('/level/getupdateTimeList').then(res => {
        if (res.data.code === 0) {
          this.updateTimeList = res.data.data;
        } else {
          this.$message.error(res.data.msg);
        }
      }).catch(e => {
