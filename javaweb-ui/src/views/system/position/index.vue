...
export default {
  ...
  methods: {
    ...
    save() {
      this.$refs['editForm'].validate((valid) => {
        if (valid) {
          ...
          this.$http[this.editForm.id ? 'put' : 'post'](this.editForm.id ? '/position/edit' : '/position/add', this.editForm).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.showEdit = false;
    ...
    remove(row) {
      if (!row) {  // 批量删除
        ...
        this.$confirm('确定要删除选中的岗位吗?', '提示', {type: 'warning'}).then(() => {
          const loading = this.$loading({lock: true});
          this.$http.delete('/position/delete/' + ids).then(res => {
            loading.close();
            if (res.data.code === 0) {
              this.$message({type: 'success', message: res.data.msg});
              this.$refs.table.reload();
    ...
    /* 导出数据 */
    exportData() {
      let array = [['岗位ID', '岗位名称', '岗位状态', '排序', '创建时间', '更新时间']];
      // 请求查询全部(不分页)的接口
      const loading = this.$loading({lock: true});
      this.$http.get('/position/index?page=1&limit=2000').then(res => {
        loading.close();
        if (res.data.code === 0) {
          res.data.data.records.forEach(d => {
            array.push([
              d.id, d.name, 
              ['正常', '停用'][d.status-1],
              d.sort,this.$util.toDateString(d.createTime), 
              this.$util.toDateString(d.updateTime),
            ]);
          });
          let sheet = XLSX.utils.aoa_to_sheet(array);
          this.$util.exportSheet(XLSX, sheet, '岗位列表');
        } else {
          this.$message.error(res.data.msg);
        }
      }).catch(e => {
